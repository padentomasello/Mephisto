...
from mephisto.providers.mturk.mturk_utils import give_worker_qualification, remove_worker_qualification
...
def direct_un_soft_block_mturk_workers(
    db: "MephistoDB",
    worker_list: List[str],
    soft_block_qual_name: str,
    requester_name: Optional[str] = None,
    provider_type: Optional[str] = "mturk",
):
    """
    Directly remove  all of the MTurk worker ids 
    in worker_list from the soft blocking MTurk qualification that Mephisto 
    associates with soft_block_qual_name. If requester_name is not provided, it will use the 
    most recently registered mturk requester in the database.
    """
    reqs = db.find_requesters(requester_name=requester_name, provider_type=provider_type)
    requester = reqs[-1]
    ...
    if mturk_qual_details is not None:
        ...
        print(
            "No existing `soft_block_qual_name` to use `direct_un_soft_block_mturk_workers; Skipping Unblocking`"
        )
        return
    ...
    for idx, worker_id in enumerate(worker_list):
        if idx % 50 == 0:
            print(f'Unblocked {idx + 1} workers so far.')
        try:
            remove_worker_qualification(mturk_client, worker_id, qualification_id)
        except Exception as e:
            print(f'Failed to unblock worker with ID: \"{worker_id}\" qualification with error: {e}. Skipping.')

def direct_allow_mturk_workers(
    db: "MephistoDB",
    worker_list: List[str],
    allowlist_qual_name: str,
    requester_name: Optional[str] = None,
):
    """
    Directly assign the soft blocking MTurk qualification that Mephisto
    ...
    for idx, worker_id in enumerate(worker_list):
        if idx % 50 == 0:
            print(f"Allow {idx + 1} workers to work on this task so far.")
        try:
            give_worker_qualification(
                mturk_client, worker_id, qualification_id, value=1
            )
